...
interface TimetableEntry {
  id: string;
  section?: { name: string; course?: { code: string } | null } | null;
...
export default function AbsenceManagement() {
  ...
  const openProcessDialog = async (absence: FacultyAbsence) => {
    ...
    const { data: classes } = await supabase
      ...
      .select(`
        id,
        day_of_week,
        section:sections(name, course:courses(code)),
  ...
  const processAbsence = async () => {
    ...
    try {
      // Update timetable entries for the affected day
      const absenceDate = new Date(selectedAbsence.absence_date);
      const dayOfWeek = absenceDate.getDay();
                    ...
                    >
                      <div>
                        <p className="font-medium">
                          {entry.section?.course?.code} - {entry.section?.name}
                        </p>
